...
class GridWidget(QWidget):
    ...
    def mousePressEvent(self, e):
        ...
        if not cell:
            ...
            self.sel = None
        elif self.main.mode == "CPU" and not self.main.human_turn:
            pass
        elif not self.sel:
            self.sel = cell
        ...
        for r in range(self.board.rows):
            for c in range(self.board.cols):
                ...
                if cell.occupied:
                    ...
                    bg, txt = QColor(THEME["ACCENT"]), Qt.GlobalColor.white
                elif cell == self.hov:
                    bg = QColor("#F5F5F5")
...
class MainWindow(QMainWindow):
    ...
    def init_menu(self):
        ...
        l.addStretch()
        l.addWidget(QLabel("DOMINOSA", objectName="Title"),
                    0, Qt.AlignmentFlag.AlignCenter)
        l.addWidget(QLabel("", objectName="Sub"),
                    0, Qt.AlignmentFlag.AlignCenter)
    ...
    def init_game(self):
        ...
        h = QHBoxLayout()
        h.setContentsMargins(30, 30, 30, 0)

        back = QPushButton("EXIT")
        back.setFixedWidth(80)
        back.clicked.connect(self.reset_to_menu)

        self.lbl_mode = QLabel("SOLO MODE")
        self.lbl_mode.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold))
        ...
        side = QVBoxLayout()
        side.setContentsMargins(0, 40, 30, 40)
        ...
        self.hint_btn = QPushButton("USE HINT")
    ...
    def start(self, mode):
        ...
        selection_text = self.sz.currentText()
        n = int(selection_text.split()[0].split('-')[1])

        try:
            mat = PuzzleDB.get_board(n)
    ...
    def break_bond(self, cell):
        edge = next((e for e in self.board.edges
                     if e.state == BondState.CONFIRMED and cell in (e.node_a, e.node_b)),
                    None)
        if edge:
    ...
    def cpu_move(self):
        ...
        if mv and mv.state != BondState.BLOCKED:
            self.commit(mv, "CPU")
        else:
            self.status.setText("CPU Passes.")
    ...
    def do_hint(self):
        ...
        if mv.state == BondState.BLOCKED:
            self.grid.flash = mv
            self.grid.flash_c = QColor(THEME["ERR"])
            self.grid.timer.start(800)
            self.status.setText("Blocking Duplicate...")
        else:
            self.grid.high = mv
            self.status.setText(f"HINT: {r}")
            QTimer.singleShot(
                2000,
                lambda: (setattr(self.grid, 'high', None), self.grid.update())
            )
        self.grid.update()
